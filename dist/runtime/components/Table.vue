...
const virtualizer = !!props.virtualize && useVirtualizer({
  ...
  measureElement: (el, entry, instance) => {
    ...
    const base = userMeasure ? userMeasure(el, entry, instance) : Math.round(entry?.borderBoxSize?.[0]?.blockSize ?? el.offsetHeight);
    ...
    (next, prev) => {
      nextTick(() => {
        ...
        changedIds.forEach((id) => {
          ...
          const el = root.querySelector(`[data-index="${index}"]`);
...
<template>
  <DefineRowTemplate v-slot="{ row, style, index }">
    <tr
      :ref="index !== void 0 ? measureRowRef : void 0"
      :data-index="index"
